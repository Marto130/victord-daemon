...
func InsertVector(vectorData *models.InsertVectorRequest, idxName string) (*uint64, error) {

	indexResource, exists := GetIndex(idxName)
	if !exists {
	...
	dims, dimsExists := GetIndexDims(idxName)
	if !dimsExists {
...
func SearchVector(vector []*float32, idxName string, topK int) (*models.SearchVectorResult, error) {
